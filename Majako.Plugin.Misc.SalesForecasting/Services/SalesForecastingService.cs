...
namespace Majako.Plugin.Misc.SalesForecasting.Services
{
    public class SalesForecastingService
    {
        ...
        public async Task<IEnumerable<ForecastResponse>> SubmitAndGetForecastAsync(int periodLength, IEnumerable<Product> products)
        {
            ...
            _pollingCancellationTokenSource.Cancel();
            _pollingCancellationTokenSource = new CancellationTokenSource();
            await PollForecastAsync(forecastId, _pollingCancellationTokenSource.Token);
        ...
        private async Task PollForecastAsync(string forecastId, CancellationToken token)
        {
